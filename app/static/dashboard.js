...
function setPeriodCurrentMonth(){ const now=new Date(); const start=new Date(now.getFullYear(), now.getMonth(), 1); const end=now; start.setHours(0,0,0,0); end.setHours(0,0,0,0); document.getElementById('start').value=toISO(start); document.getElementById('end').value=toISO(end); }
function setPeriodPrevMonth(){ const now=new Date(); const first=new Date(now.getFullYear(), now.getMonth(), 1); const prevLast=new Date(first-1); const prevFirst=new Date(prevLast.getFullYear(), prevLast.getMonth(), 1); prevFirst.setHours(0,0,0,0); prevLast.setHours(0,0,0,0); document.getElementById('start').value=toISO(prevFirst); document.getElementById('end').value=toISO(prevLast); }
